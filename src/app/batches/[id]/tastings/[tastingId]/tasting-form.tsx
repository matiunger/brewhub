...
function SegmentedScale({
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex gap-1">
        {Array.from({ length: max + 1 }, (_, i) => (
          <button
            ...
            type="button"
            onClick={() => onChange(i)}
            className={`h-7 min-w-[2rem] px-1.5 rounded text-xs border transition-colors ${
              value === i
      ...
      {onInappropriateChange && (
        <label className="flex items-center gap-1 ml-auto text-xs text-muted-foreground cursor-pointer whitespace-nowrap">
          <input
            type="checkbox"
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <Select value={data.appearance.color || "none"} onValueChange={(v) => patchAppearance("color", v === "none" ? "" : v)}>
              <SelectTrigger className="h-8 text-sm w-44"><SelectValue placeholder="Select…" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                {["Very pale", "Pale", "Gold", "Amber", "Brown", "Dark"].map((c) => (
                  <SelectItem key={c} value={c.toLowerCase()}>{c}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            <label className="flex items-center gap-1 ml-auto text-xs text-muted-foreground cursor-pointer whitespace-nowrap">
              <input type="checkbox" checked={data.appearance.inappropriateProps.includes("color")} onChange={(e) => setInap("appearance", "color", e.target.checked)} className="h-3.5 w-3.5" />
              Inappropriate
            </label>
          </div>
          <div className="flex items-center gap-3">
            ...
            <label className="flex items-center gap-1 ml-auto text-xs text-muted-foreground cursor-pointer whitespace-nowrap">
              <input type="checkbox" checked={data.appearance.inappropriateProps.includes("clarity")} onChange={(e) => setInap("appearance", "clarity", e.target.checked)} className="h-3.5 w-3.5" />
              Inappropriate
            </label>
          </div>
          <SegmentedScale label="Head size" value={data.appearance.headSize} max={3} labels={["None", "Low", "Med", "High"]} onChange={(v) => patchAppearance("headSize", v)} inappropriate={data.appearance.inappropriateProps.includes("headSize")} onInappropriateChange={(c) => setInap("appearance", "headSize", c)} />
          <SegmentedScale label="Head retention" value={data.appearance.headRetention} max={3} labels={["None", "Low", "Med", "High"]} onChange={(v) => patchAppearance("headRetention", v)} inappropriate={data.appearance.inappropriateProps.includes("headRetention")} onInappropriateChange={(c) => setInap("appearance", "headRetention", c)} />
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground w-28 shrink-0">Head color</span>
            <Select value={data.appearance.headColor || "none"} onValueChange={(v) => patchAppearance("headColor", v === "none" ? "" : v)}>
              <SelectTrigger className="h-8 text-sm w-44"><SelectValue placeholder="Select…" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                {["White", "Off-white", "Cream", "Tan"].map((c) => (
                  <SelectItem key={c} value={c.toLowerCase()}>{c}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            <label className="flex items-center gap-1 ml-auto text-xs text-muted-foreground cursor-pointer whitespace-nowrap">
              <input type="checkbox" checked={data.appearance.inappropriateProps.includes("headColor")} onChange={(e) => setInap("appearance", "headColor", e.target.checked)} className="h-3.5 w-3.5" />
              Inappropriate
            </label>
          </div>
          <input
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          <SegmentedScale label="Malt" value={data.aroma.malt} max={3} labels={["None", "Low", "Med", "High"]} onChange={(v) => patchAroma("malt", v)} inappropriate={data.aroma.inappropriateProps.includes("malt")} onInappropriateChange={(c) => setInap("aroma", "malt", c)} />
          <SegmentedScale label="Hops" value={data.aroma.hops} max={3} labels={["None", "Low", "Med", "High"]} onChange={(v) => patchAroma("hops", v)} inappropriate={data.aroma.inappropriateProps.includes("hops")} onInappropriateChange={(c) => setInap("aroma", "hops", c)} />
          <SegmentedScale label="Fermentation" value={data.aroma.fermentation} max={3} labels={["None", "Low", "Med", "High"]} onChange={(v) => patchAroma("fermentation", v)} inappropriate={data.aroma.inappropriateProps.includes("fermentation")} onInappropriateChange={(c) => setInap("aroma", "fermentation", c)} />

          <div className="pt-2">
            <p className="text-xs font-medium text-muted-foreground mb-2">Descriptors</p>
          ...
          <div className="pt-2 space-y-2">
            <SegmentedScale label="Sweetness" value={data.flavor.sweetness} max={3} labels={["None", "Low", "Med", "High"]} onChange={(v) => patchFlavor("sweetness", v)} inappropriate={data.flavor.inappropriateProps.includes("sweetness")} onInappropriateChange={(c) => setInap("flavor", "sweetness", c)} />
            <SegmentedScale label="Bitterness" value={data.flavor.bitterness} max={3} labels={["None", "Low", "Med", "High"]} onChange={(v) => patchFlavor("bitterness", v)} inappropriate={data.flavor.inappropriateProps.includes("bitterness")} onInappropriateChange={(c) => setInap("flavor", "bitterness", c)} />
            <SegmentedScale label="Acidity" value={data.flavor.acidity} max={3} labels={["None", "Low", "Med", "High"]} onChange={(v) => patchFlavor("acidity", v)} inappropriate={data.flavor.inappropriateProps.includes("acidity")} onInappropriateChange={(c) => setInap("flavor", "acidity", c)} />
            <BipolarScale label="Balance" leftLabel="Hoppy" rightLabel="Malty" value={data.flavor.balance} max={4} onChange={(v) => patchFlavor("balance", v)} inappropriate={data.flavor.inappropriateProps.includes("balance")} onInappropriateChange={(c) => setInap("flavor", "balance", c)} />
            <BipolarScale label="Finish" leftLabel="Dry" rightLabel="Sweet" value={data.flavor.finish} max={4} onChange={(v) => patchFlavor("finish", v)} inappropriate={data.flavor.inappropriateProps.includes("finish")} onInappropriateChange={(c) => setInap("flavor", "finish", c)} />
          </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          <SegmentedScale label="Body" value={data.mouthfeel.body} max={3} labels={["None", "Low", "Med", "High"]} onChange={(v) => patchMouthfeel("body", v)} inappropriate={data.mouthfeel.inappropriateProps.includes("body")} onInappropriateChange={(c) => setInap("mouthfeel", "body", c)} />
          <SegmentedScale label="Carbonation" value={data.mouthfeel.carbonation} max={3} labels={["None", "Low", "Med", "High"]} onChange={(v) => patchMouthfeel("carbonation", v)} inappropriate={data.mouthfeel.inappropriateProps.includes("carbonation")} onInappropriateChange={(c) => setInap("mouthfeel", "carbonation", c)} />
          <SegmentedScale label="Warmth" value={data.mouthfeel.warmth} max={3} labels={["None", "Low", "Med", "High"]} onChange={(v) => patchMouthfeel("warmth", v)} inappropriate={data.mouthfeel.inappropriateProps.includes("warmth")} onInappropriateChange={(c) => setInap("mouthfeel", "warmth", c)} />
          <SegmentedScale label="Creaminess" value={data.mouthfeel.creaminess} max={3} labels={["None", "Low", "Med", "High"]} onChange={(v) => patchMouthfeel("creaminess", v)} inappropriate={data.mouthfeel.inappropriateProps.includes("creaminess")} onInappropriateChange={(c) => setInap("mouthfeel", "creaminess", c)} />
          <SegmentedScale label="Astringency" value={data.mouthfeel.astringency} max={3} labels={["None", "Low", "Med", "High"]} onChange={(v) => patchMouthfeel("astringency", v)} inappropriate={data.mouthfeel.inappropriateProps.includes("astringency")} onInappropriateChange={(c) => setInap("mouthfeel", "astringency", c)} />
          <input
            type="text"
            placeholder="Other notes…"
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          <BipolarScale label="Style accuracy" leftLabel="Off" rightLabel="On" value={data.overall.styleAccuracy} max={4} onChange={(v) => patchOverall("styleAccuracy", v)} inappropriate={data.overall.inappropriateProps.includes("styleAccuracy")} onInappropriateChange={(c) => setInap("overall", "styleAccuracy", c)} />
          <BipolarScale label="Technical quality" leftLabel="Poor" rightLabel="Excellent" value={data.overall.technicalQuality} max={4} onChange={(v) => patchOverall("technicalQuality", v)} inappropriate={data.overall.inappropriateProps.includes("technicalQuality")} onInappropriateChange={(c) => setInap("overall", "technicalQuality", c)} />
          <BipolarScale label="Intangibles" leftLabel="Low" rightLabel="High" value={data.overall.intangibles} max={4} onChange={(v) => patchOverall("intangibles", v)} inappropriate={data.overall.inappropriateProps.includes("intangibles")} onInappropriateChange={(c) => setInap("overall", "intangibles", c)} />
          <textarea
